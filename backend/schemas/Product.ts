...
whenever product will be created it will connect
this user field to whomever that logged in at that time.
...
who have permission to do that or
they own the product by rules.canManageProducts


🤔 : update: rules.canManageProducts
   ~ update : { user: { id: session.itemId } }
// where filter like graphql but at access level


-it will check and filter all the product
with product user id and if it will match
...
> if user have permission or if it's in "AVAILABLE" mode
 only then they can read it.

- test all of this in
admin ui or
at frontend or
at graphql api
with two different logged in users.
